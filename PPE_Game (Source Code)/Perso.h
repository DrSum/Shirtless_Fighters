...
typedef struct Perso
{
    ...
    */
    ...
    int InitNbDeath()
    {
        int nbDeath=0;
        FILE* fichier = NULL;
        fichier = fopen("data/nbMorts.txt","r");
        fscanf(fichier,"%d",&nbDeath);
        fclose(fichier);
        return nbDeath;
    }

    void updateDeath(int nbDead)
    {
        FILE* fichier = NULL;
        fichier = fopen("data/nbMorts.txt","r+");
        fprintf(fichier, "%d", nbDead);
        fclose(fichier);
    }
    ...
    void update(float deltaTime, int matMap[40][23], std::vector<Perso> *perso)
    {
        ...
        calculateColliderPos();
        gun.actuBullet(Right * 32, Left * 32, matMap,direction);
        ...
        if(isAttacking)
        {
            ...
            for(unsigned int i = 0; i < perso->size(); i++)
            {
                ...
                if(ImgGetCollision(weapon.getCurrentImage(), perso->at(i).getImage()))
                {
                    //if it's not the player itself, and if the player is not attacking a invincible player
            ...
            if(Time() >= animTimeWeapon && isAttacking)
            {
                isAttacking = false;
                canAttack=false;
            }
        }
        else if(Time() >= animTimeWeapon+400 && !isInvicible)
        {
            canAttack=true;
        }
            for(unsigned int i = 0; i < perso->size(); i++)
            {
                ...
                for (int j = 0; j < 4; j++)
                {
                    if(ImgGetCollision(gun.bullet[j], perso->at(i).getImage()) && !perso->at(i).isInvicible)
                    {
                        Scored();
                        perso->at(i).Die(getRandomSpawnPoint(*respawnPoint, respawnPointTimer));
                        perso->at(i).gun.reload();
    ...
    void Shoot()
    {
        if(canAttack)
        {
            ...
            {
                gun.Bullet(2, Right * 32, Left * 32);
                isShooting=true;
                animShoot=Time()+150;
            }
            else if(movementDirection==DOWN)
            {
                gun.Bullet(3, Right * 32, Left * 32);
                isShooting=true;
                animShoot=Time()+150;
            }
            else
            {
                gun.Bullet(direction, Right * 32, Left * 32);
                isShooting=true;
                animShoot=Time()+150;
            }
        }
    ...
    //make spike effect working
    bool checkLeftSpikeCollision(int matMap[40][23])
    {
        if (matMap[(int)(Left.x)][(int)(Left.y)] == 3 || matMap[(int)(topLeft.x)][(int)(topLeft.y)] == 3)
        {
            return true;
        }
        else
    ...
    bool checkTopSpikeCollision(int matMap[40][23])
    {
        if(matMap[(int)(topLeft.x)][(int)(topLeft.y)] == 3 || matMap[(int)(topRight.x)][(int)(topRight.y)] == 3)
        {
            return true;
        }
        else
    ...
    bool checkBottomSpikeCollision(int matMap[40][23])
    {
        if (matMap[(int)(bottomLeft.x)][(int)(bottomLeft.y)] == 3 || matMap[(int)(bottomRight.x)][(int)(bottomRight.y)] == 3)
        {
            return true;
        }
        else
    ...
    bool checkRightSpikeCollision(int matMap[40][23])
    {

        if (matMap[(int)(Right.x)][(int)(Right.y)] == 3 || matMap[(int)(topRight.x)][(int)(topRight.y)] == 3)
        {
            return true;
        }
        else
    ...
    //Make water effect working


    bool checkWaterCollision(int matMap[40][23])
    {

        if (matMap[(int)(((Right.x*32)-16)/32)][(int)(Right.y)] == 4 )
        {
            return true;
        }
        else
